...
| 7 | No speculative abstraction: one implementation needs no interface. Add one when a second consumer or implementation exists. | Single-impl interfaces; unused generics, parameters, factories, extension points |
...
| Speculative flexibility: single-impl interface, unused params, factory for one variant | YAGNI | Delete until a second consumer or implementation exists | No interface with exactly one implementation; no unused extension points |
...
5. Is every interface implemented by exactly one class with one caller? (YAGNI — remove it)
...
| Mistake | Correction |
|---|---|
| Routing "fat interface" to ISP when it has one implementer | That is YAGNI: delete the interface or its unused methods until a second real consumer exists |
...
| Adding interfaces "for mockability" before tests exist | Write the test first; the interface appears only when a genuine second implementation does |
