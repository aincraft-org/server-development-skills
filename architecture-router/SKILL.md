...
| 10 | Draw the thread boundary with blocking I/O and main-thread-only APIs: blocking I/O and heavy computation run off the main thread, and Bukkit/main-thread-only APIs stay on it. Async orchestration (CompletableFuture, scheduler) inside services is legitimate — the boundary is the thread the blocking work actually runs on, not where the async call is composed (see [performance-optimization](../performance-optimization/SKILL.md)). | Blocking I/O, sleeps, or heavy computation executing on the main thread; Bukkit/main-thread-only APIs called from async contexts |
