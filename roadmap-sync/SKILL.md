...
```
[ server-roadmap.ods ]  (Master spreadsheet edited via Calc/LibreOffice)
         │
...
### 3. Sync and Push to GitHub (Explicit Opt-in)
...
The roadmap synchronizer parses and normalizes the following columns:

| Field | Type | Description | Example |
| :--- | :--- | :--- | :--- |
| `Plugin` | `string` | Plugin identifier / project name | `Mint`, `Guilds`, `Omnia` |
| `Version` | `enum` | Target release milestone (`V1`, `V2`) | `V1` |
| `Developer` | `string` | Lead developer or `3rd Party` | `mintychochip`, `ChaosInferno` |
| `Responsibilities` | `string` | Summary of feature scope and purpose | `Economy`, `Territory Management` |
...
| :--- | :--- | :--- |
| Editing `server-roadmap.csv` directly | Overwritten on next `.ods` sync | Always edit `server-roadmap.ods` as the source of truth, then run `sync-roadmap.py`. |
| Missing LibreOffice for XLSX conversion | XLSX export fails or stays stale | Verify `libreoffice --version` is present; script outputs a clear warning if conversion fails. |
